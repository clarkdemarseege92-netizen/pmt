// 文件: /app/[locale]/merchant/design/page.tsx (Client Component)
"use client";
...
export default function MerchantDesignPage() {
  const t = useTranslations('merchantDesign');
  ...
  if (loading) return (
    <div className="min-h-screen bg-base-200 flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="mt-4 text-lg">{t('loadingMerchant')}</p>
      </div>
    </div>
  );
  ...
  if (!merchantId) return (
    <div className="min-h-screen bg-base-200 flex items-center justify-center">
      <div className="text-center max-w-md p-8">
        ...
        <h2 className="text-2xl font-bold mb-2">{t('noMerchantFound')}</h2>
        <p className="text-base-content/70 mb-6">
          {t('noMerchantDescription')}
        </p>
      </div>
    </div>
  );
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            <HiPaintBrush className="text-primary" /> {t('title')}
          </h1>
        ...
        <div className="flex gap-3">
          {merchantId && (
            <Link
              href={`/shop/${merchantId}` as `/shop/${string}`}
              target="_blank"
          ...
          <button
            ...
            disabled={saving}
          >
            {saving ? <span className="loading loading-spinner"></span> : <><HiCheck /> {t('savePublish')}</>}
          </button>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="card bg-base-100 shadow-sm">
            <div className="card-body">
              <h2 className="card-title text-lg mb-4"><HiSwatch /> {t('styleSettings')}</h2>
              <div className="grid md:grid-cols-2 gap-6">
                <div className="form-control">
                  <label className="label"><span className="label-text font-bold">{t('primaryColor')}</span></label>
                  ...
                  <span className="text-xs text-base-content/50 mt-1">{t('primaryColorHint')}</span>
                </div>

                <div className="form-control">
                  <label className="label"><span className="label-text font-bold">{t('buttonStyle')}</span></label>
          ...
          <div className="card bg-base-100 shadow-sm">
            <div className="card-body">
              <h2 className="card-title text-lg mb-4"><HiPhoto /> {t('images')}</h2>

              <div className="form-control mb-4">
                <label className="label"><span className="label-text font-bold">{t('coverImage')}</span></label>
              ...
              <div className="form-control">
                <label className="label"><span className="label-text font-bold">{t('backgroundImage')}</span></label>
          ...
          <div className="card bg-base-100 shadow-sm">
            <div className="card-body">
              <h2 className="card-title text-lg mb-4"><HiArrowTopRightOnSquare /> {t('layout')}</h2>
              ...
              <div className="form-control">
                <label className="label cursor-pointer justify-start gap-4">
                  ...
                  />
                  <span className="label-text">{t('showStock')}</span>
                </label>
              </div>
              ...
              <div className="form-control">
                <label className="label cursor-pointer justify-start gap-4">
                  ...
                  />
                  <span className="label-text">{t('showSalesCount')}</span>
                </label>
              </div>

              <div className="form-control mt-4">
                <label className="label"><span className="label-text">{t('productGrid')}</span></label>
        ...
        <div className="lg:col-span-1">
          <div className="sticky top-24">
            <div className="flex items-center gap-2 mb-2 font-bold text-sm opacity-50 justify-center">
              <HiDevicePhoneMobile /> {t('livePreview')}
            </div>
                      ...
                      <div key={i} className="bg-white p-2 rounded-lg shadow-sm flex flex-col gap-2">
                        ...
                        {(config.display_config.show_stock || config.display_config.show_sales_count) && (
                          <div className="text-[10px] text-gray-400 flex gap-1">
                            {config.display_config.show_stock && <span>{t('previewStock')}</span>}
                            {config.display_config.show_sales_count && <span>{t('previewSold')}</span>}
                          </div>
                        )}
                      </div>
                    ))}
