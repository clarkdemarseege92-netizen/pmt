// 文件: /app/merchant/orders/MerchantOrdersClient.tsx
"use client";
...
const statusBadge = (status: string) => {
  switch (status) {
    case 'paid': return <span className="badge badge-success text-white gap-1"><HiCheckCircle/> 已支付/待核销</span>;
    case 'pending': return <span className="badge badge-warning gap-1"><HiClock/> 待支付</span>;
    case 'used': return <span className="badge badge-neutral gap-1">已核销</span>;
    case 'expired': return <span className="badge badge-error text-white gap-1"><HiXCircle/> 已过期</span>;
  ...
  return (
    <div>
      ...
      <div role="tablist" className="tabs tabs-boxed mb-6 bg-base-200 p-1 rounded-lg inline-flex">
        ...
        >
          <HiShoppingBag className="mr-2" /> 购物车商品订单 ({cartOrders.length})
        </a>
        <a 
          role="tab" 
        ...
        >
          <HiTicket className="mr-2" /> 优惠券/套餐订单 ({couponOrders.length})
        </a>
      </div>
      ...
      <div className="space-y-4">
        {displayOrders.length === 0 ? (
          <div className="text-center py-12 text-base-content/50 bg-base-100 rounded-xl border border-base-200">
            暂无{activeTab === 'cart' ? '购物车' : '优惠券'}订单数据
          </div>
        ) : (
              ...
              <div className="card-body p-5">
                ...
                <div className="flex justify-between items-start mb-4 border-b border-base-100 pb-3">
                  <div className="flex flex-col gap-1">
                    <span className="text-xs text-base-content/50 font-mono">订单号: {order.order_id.slice(0, 8)}...</span>
                ...
                <div className="flex flex-col md:flex-row gap-6">
                    ...
                    {activeTab === 'cart' ? (
                        ...
                        <div key={idx} className="flex gap-3 items-center bg-base-50 p-2 rounded-lg">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="text-xs text-base-content/60">
                              单价: ฿{item.products?.original_price} × <span className="font-bold text-base-content">{item.quantity}</span>
                            </div>
                          </div>
                        </div>
                      ))
                      ...
                      <div className="flex gap-3 items-center bg-base-50 p-2 rounded-lg">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-xs badge badge-outline badge-sm mt-1">优惠套餐</div>
                        </div>
                      </div>
                    )}
                  </div>
                  ...
                  <div className="md:w-64 md:border-l md:border-base-200 md:pl-6 flex flex-col justify-center gap-2 text-sm">
                    <div className="flex items-center gap-2 text-base-content/80" title="客户联系方式">
                      <HiUser className="w-4 h-4" />
                      <span>{order.profiles?.phone || order.profiles?.email || "未知客户"}</span>
                    </div>
                    
                    <div className="bg-base-200/50 p-2 rounded text-center">
                      <div className="text-xs text-base-content/50 mb-1">核销码 (Redeem Code)</div>
