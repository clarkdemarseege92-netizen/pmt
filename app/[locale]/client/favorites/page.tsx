// app/[locale]/client/favorites/page.tsx
...
interface ProductRaw {
  product_id: string;
  name: string | { th: string; en: string; zh: string };
...
export default async function FavoritesPage({
  ...
  const favoriteProducts = ((productsRes.data as ProductRaw[]) || []).map((product) => ({
    ...
    name: getLocalizedValue(product.name, locale as 'th' | 'zh' | 'en'),
  ...
  const favoriteCoupons = ((couponsRes.data as CouponRaw[]) || []).map((coupon) => ({
    ...
    title: getLocalizedValue(coupon.name, locale as 'th' | 'zh' | 'en'),
    description: `${coupon.original_value > 0 ? `฿${coupon.original_value}` : ''} | ฿${coupon.selling_price}`,
