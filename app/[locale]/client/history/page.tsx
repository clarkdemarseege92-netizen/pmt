// app/[locale]/client/history/page.tsx
  ...
  if (!history || history.length === 0) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/" className="btn btn-primary btn-sm mt-6">{t('empty.goHome')}</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <h1 className="text-3xl font-bold flex items-center gap-2">
        <HiClock className="text-secondary" /> {t('title')}
      </h1>
      ...
      <div className="space-y-2">
        {history.map(item => {
          ...
          if (item.item_type === 'product') {
            ...
            label = t('types.coupon');
            icon = <HiTag />;
          } else if (item.item_type === 'merchant') {
            detail = merchantsMap.get(item.item_id);
            link = `/shop/${item.item_id}`;
            label = t('types.merchant');
            icon = <HiBuildingStorefront />;
          }

          if (!detail) return null;

          const name = item.item_type === 'merchant'
            ? detail.shop_name
            : getLocalizedValue(detail.name, locale as 'th' | 'zh' | 'en');
          const image = item.item_type === 'merchant' ? detail.logo_url : detail.image_urls?.[0];
          const time = new Date(item.viewed_at).toLocaleString(locale, { month: 'short', day: 'numeric', hour: '2-digit', minute:'2-digit' });
